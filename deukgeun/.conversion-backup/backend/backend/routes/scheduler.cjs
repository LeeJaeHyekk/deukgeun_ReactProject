...
const { schedulerAPI  } = require('../services/autoUpdateScheduler');
const { DataReferenceService  } = require('../services/dataReferenceService');
...
router.get("/data-reference-stats", async (req, res) => {
    try {
        ...
        const stats = await DataReferenceService.checkAllGymsReferenceStatus(gymRepo);
...
router.get("/gym/:id/data-reference", async (req, res) => {
    try {
        ...
        const connection = await connectDatabase();
        const gymRepo = connection.getRepository(Gym);
        const status = await DataReferenceService.checkDataReferenceStatus(gymRepo, gymId);
        await connection.close();
        res.json({ success: true, data: status });
    }
    catch (error) {
...
router.get("/gyms-needing-update", async (req, res) => {
    try {
        const connection = await connectDatabase();
        const gymRepo = connection.getRepository(Gym);
        const gymsNeedingUpdate = await DataReferenceService.getGymsNeedingUpdate(gymRepo);
        await connection.close();
        res.json({
            success: true,
            data: {
