...
const { WorkoutStats  } = require('../entities/WorkoutStats');
const { WorkoutProgress  } = require('../entities/WorkoutProgress');
const { WorkoutReminder  } = require('../entities/WorkoutReminder');
...
module.exports.seedOptimizedData = seedOptimizedData
module.exports.seedOptimizedData = seedOptimizedData);
        ...
        for (let i = 0; i < 15; i++) {
            ...
            const workoutSession = workoutSessionRepository.create({
                ...
                endTime: new Date(Date.now() - i * 24 * 60 * 60 * 1000 + 60 * 60 * 1000), // 1시간 후
        ...
        const workoutProgressRepository = AppDataSource.getRepository(WorkoutProgress);
        for (const user of createdUsers.slice(0, 3)) {
            ...
            for (const machine of createdMachines.slice(0, 5)) {
                ...
                for (let i = 0; i < 3; i++) {
                    // 각 기구당 3개의 진행도 기록
                    const progressDate = new Date(Date.now() - i * 7 * 24 * 60 * 60 * 1000); // 최근 3주간
        ...
        const workoutReminderRepository = AppDataSource.getRepository(WorkoutReminder);
        for (const user of createdUsers.slice(0, 3)) {
            // 처음 3명의 사용자만
