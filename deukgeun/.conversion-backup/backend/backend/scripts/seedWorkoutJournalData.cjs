...
exports.seedWorkoutJournalData = seedWorkoutJournalData;
...
async function seedWorkoutJournalData() {
    ...
    try {
        ...
        if (!user) {
            console.log("❌ 테스트 사용자를 찾을 수 없습니다. 먼저 사용자를 생성해주세요.");
            return;
        }
        ...
        const goalRepository = databaseConfig_1.AppDataSource.getRepository(WorkoutGoal_1.WorkoutGoal);
        const sampleGoals = [
            {
                userId: user.id,
        ...
        for (const goalData of sampleGoals) {
            ...
            console.log(`✅ 운동 목표 생성: ${savedGoal.type} - ${savedGoal.targetValue}${savedGoal.unit}`);
        }
        const sessionRepository = databaseConfig_1.AppDataSource.getRepository(WorkoutSession_1.WorkoutSession);
        const setRepository = databaseConfig_1.AppDataSource.getRepository(ExerciseSet_1.ExerciseSet);
        const recentDates = [];
        for (let i = 0; i < 14; i++) {
            const date = new Date();
        ...
        const statsRepository = databaseConfig_1.AppDataSource.getRepository(WorkoutStats_1.WorkoutStats);
        for (let i = 0; i < 7; i++) {
            const date = new Date();
            date.setDate(date.getDate() - i);
            const statsData = {
        ...
        const progressRepository = databaseConfig_1.AppDataSource.getRepository(WorkoutProgress_1.WorkoutProgress);
        for (let i = 0; i < 10; i++) {
            const date = new Date();
