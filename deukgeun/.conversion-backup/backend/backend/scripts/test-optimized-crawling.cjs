...
const OptimizedCrawlingService_1 = require('modules/crawling/core/OptimizedCrawlingService');
...
async function testSearchEngineFactory() {
    const factory = new SearchEngineFactory_1.SearchEngineFactory({
        timeout: 10000,
        delay: 500,
        maxRetries: 2,
        enableParallel: true,
...
async function testUnifiedDataMerger() {
    const merger = new UnifiedDataMerger_1.UnifiedDataMerger();
    const originalData = [
        {
            id: 1,
            name: "페이즈짐",
...
async function testOptimizedCrawlingService() {
    const mockRepo = new MockGymRepository();
    const service = new OptimizedCrawlingService_1.OptimizedCrawlingService(mockRepo);
    service.updateConfig({
        ...
        searchEngines: ['naver_cafe', 'naver'],
...
async function testPerformanceBenchmark() {
    ...
    const parallelPromises = testQueries.map(query => factory.searchWithEngine('naver_cafe', query.name, query.address).catch(() => null));
