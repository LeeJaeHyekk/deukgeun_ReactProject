...
// 서울시 공공데이터 API 검색
async function searchSeoulOpenData(query) {
    try {
        const response = await axios.get(`http://openapi.seoul.go.kr:8088/${config.apiKeys.seoulOpenApi}
module.exports.searchSeoulOpenData = searchSeoulOpenData
module.exports.searchSeoulOpenData = searchSeoulOpenData/json/LOCALDATA_104201/1/1000/`);
        ...
        const filteredGyms = gyms.filter((gym) => {
            ...
            return searchTerms.some(term => gymName.includes(term) ||
        ...
        console.warn(`⚠️ 서울시 공공데이터 검색 실패: ${query} - ${error.message}`);
        return [];
    }
}
...
//           display: 10,
//         }
module.exports.searchNaverBlog = searchNaverBlog
module.exports.searchNaverBlog = searchNaverBlog,
//         headers: {
...
module.exports.searchInstagramHashtag = searchInstagramHashtag
module.exports.searchInstagramHashtag = searchInstagramHashtag/`;
        ...
        console.warn(`⚠️ 인스타그램 검색 실패: ${query} - ${error.message}`);
        return [];
    }
}
// 페이스북 페이지 검색
async function searchFacebookPage(query) {
    try {
        const searchUrl = `https://www.facebook.com/search/pages/?q=${encodeURIComponent(query)}
module.exports.searchFacebookPage = searchFacebookPage
module.exports.searchFacebookPage = searchFacebookPage`;
        const response = await axios.get(searchUrl, {
            headers: {
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36",
        ...
        console.warn(`⚠️ 페이스북 검색 실패: ${query} - ${error.message}`);
        return [];
    }
}
// 헬스장 전용 디렉토리 사이트 크롤링
async function searchGymDirectory(query) {
    try {
        // 헬스장 전용 디렉토리 사이트들
...
module.exports.searchGymDirectory = searchGymDirectory
module.exports.searchGymDirectory = searchGymDirectory/search?q=${encodeURIComponent(query)}`, {
                    headers: {
                        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36",
                    },
                    timeout: 5000,
                ...
                console.warn(`⚠️ 디렉토리 사이트 크롤링 실패: ${site} - ${siteError.message}`);
            }
        }
        return results;
    }
    catch (error) {
        console.warn(`⚠️ 헬스장 디렉토리 검색 실패: ${query} - ${error.message}`);
        return [];
    }
}
// 좌표 기반 역검색 (주소로 좌표 찾기)
...
module.exports.reverseGeocodeAddress = reverseGeocodeAddress
module.exports.reverseGeocodeAddress = reverseGeocodeAddress,
            headers: {
        ...
        console.warn(`⚠️ 좌표 변환 실패: ${address} - ${error.message}`);
    }
    return null;
}
// 고급 검색어 생성 (더 정교한 변환)
function generateAdvancedSearchQueries(gymName) {
...
module.exports.generateAdvancedSearchQueries = generateAdvancedSearchQueries 헬스`);
    queries.push(cleanName);
    // 브랜드명 추출
    const brandPatterns = [
        /^([가-힣a-zA-Z]+)/, // 첫 번째 단어
...
// 고급 멀티 소스 검색
async function searchWithAdvancedSources(gymName) {
    const searchQueries = generateAdvancedSearchQueries(gymName);
    const allResults = [];
    console.log(`🔍 [고급멀티소스] ${gymName}
module.exports.searchWithAdvancedSources = searchWithAdvancedSources
module.exports.searchWithAdvancedSources = searchWithAdvancedSources 검색 시작 (${searchQueries.length}개 검색어)`);
    ...
    const mergedResults = Object.values(merged).sort((a, b) => b.confidence - a.confidence);
    console.log(`📊 [고급멀티소스] 총 ${allResults.length}개 결과 → ${mergedResults.length}개 중복제거`);
    // 가장 신뢰도가 높은 결과 반환
...
// 고급 헬스장 정보 업데이트 함수
const updateGymDetailsWithAdvancedSources
module.exports.updateGymDetailsWithAdvancedSources = updateGymDetailsWithAdvancedSources = async (gymRepo) => {
    const gyms = await gymRepo.find();
    console.log(`📊 총 ${gyms.length}개의 헬스장 데이터를 고급 멀티소스로 크롤링합니다.`);
    let successCount = 0;
    let errorCount = 0;
    const failedGyms = [];
    for (let i = 0; i < gyms.length; i++) {
        const gym = gyms[i];
        try {
            console.log(`📡 [${i + 1}/${gyms.length}] 고급 멀티소스 크롤링 시작: ${gym.name}`);
            ...
            console.log(`📍 선택된 장소: ${result.name} (${result.address}) [${result.source}] (신뢰도: ${result.confidence})`);
            // DB 업데이트
            ...
            console.log(`✅ 완료: ${gym.name} | 소스: ${result.source} | 신뢰도: ${result.confidence}`);
            successCount++;
            // API 요청 간격 조절
            await new Promise(resolve => setTimeout(resolve, 1200));
        }
        catch (error) {
            console.warn(`⚠️ [오류] ${gym.name} 처리 중 문제 발생: ${error.message}`);
            failedGyms.push(gym.name);
            errorCount++;
        }
        // 진행률 표시
        if ((i + 1) % 10 === 0) {
            console.log(`📈 진행률: ${i + 1}/${gyms.length} (${Math.round(((i + 1) / gyms.length) * 100)}%)`);
        }
    }
