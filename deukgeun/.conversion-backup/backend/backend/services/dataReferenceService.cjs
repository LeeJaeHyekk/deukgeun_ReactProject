...
module.exports.DataReferenceService = DataReferenceService {
    /**
     * 헬스장 데이터의 참조 상태를 확인
     */
    static async checkDataReferenceStatus(gymRepo, gymId) {
        ...
        const daysUntilNextUpdate = Math.max(0, this.UPDATE_INTERVAL_DAYS - daysSinceLastUpdate);
        const isUsingReferenceData = daysSinceLastUpdate > 0 && daysSinceLastUpdate <= this.REFERENCE_DAYS;
        // 데이터 소스 판단
     ...
     */
    static async checkAllGymsReferenceStatus(gymRepo) {
     ...
     * 업데이트가 필요한 헬스장 목록 조회
     */
    static async getGymsNeedingUpdate(gymRepo) {
        const gyms = await gymRepo.find();
        const needsUpdate = [];
        for (const gym of gyms) {
            const status = await this.checkDataReferenceStatus(gymRepo, gym.id);
     ...
     * 데이터 참조 통계 로깅
     */
    static async logDataReferenceStatistics(gymRepo) {
        ...
        if (stats.needsUpdateCount > 0) {
            logger.info(`🔄 ${stats.needsUpdateCount}개 헬스장이 업데이트를 기다리고 있습니다.`);
        }
        else {
     ...
     * 스케줄러 실행 전 상태 확인
     */
    static async preSchedulerCheck(gymRepo) {
        const stats = await this.checkAllGymsReferenceStatus(gymRepo);
        // 업데이트가 필요한 헬스장이 있거나, 마지막 업데이트가 3일 이상 된 경우
        const shouldRunUpdate = stats.needsUpdateCount > 0 ||
...
DataReferenceService.UPDATE_INTERVAL_DAYS = 3;
DataReferenceService.REFERENCE_DAYS = 2;
