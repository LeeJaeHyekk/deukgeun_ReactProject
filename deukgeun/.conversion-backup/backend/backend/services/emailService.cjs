    ...
    async sendEmail(emailContent) {
        try {
            if (!this.isConfigured || !this.transporter) {
                ...
                logger.info(`[MOCK EMAIL] To: ${emailContent.to}, Subject: ${emailContent.subject}`);
                logger.info(`[MOCK EMAIL] Content: ${emailContent.text || emailContent.html}`);
                return true;
            }
            ...
            logger.info(`Email sent successfully to ${emailContent.to}: ${result.messageId}`);
            return true;
        }
        catch (error) {
