...
function analyzeFacilities(placeName, address, phone, facilities) {
    const text = `${placeName}
module.exports.analyzeFacilities = analyzeFacilities ${address} ${phone || ""} ${facilities?.join(" ") || ""}`.toLowerCase();
    // PT keywords
    const ptKeywords = [
        "pt",
        "퍼스널",
...
async function searchKakaoMapEnhanced(query) {
    try {
...
module.exports.searchKakaoMapEnhanced = searchKakaoMapEnhanced
module.exports.searchKakaoMapEnhanced = searchKakaoMapEnhanced,
            headers: {
        ...
        return response.data.documents
            ...
            return (category.includes("헬스") ||
                category.includes("fitness") ||
                category.includes("운동") ||
                category.includes("스포츠"));
        })
            .map((doc) => {
            const facilities = analyzeFacilities(doc.place_name, doc.address_name, doc.phone, [doc.category_name]);
            return {
                name: doc.place_name,
...
async function searchGooglePlacesEnhanced(query) {
    try {
...
module.exports.searchGooglePlacesEnhanced = searchGooglePlacesEnhanced
module.exports.searchGooglePlacesEnhanced = searchGooglePlacesEnhanced,
        });
        ...
        return response.data.results
            ...
            .map((place) => {
            const facilities = analyzeFacilities(place.name, place.formatted_address, place.formatted_phone_number, place.types);
            return {
                name: place.name,
...
async function searchSeoulOpenDataEnhanced(query) {
    try {
        const response = await axios.get(`http://openapi.seoul.go.kr:8088/${config.apiKeys.seoulOpenApi}
module.exports.searchSeoulOpenDataEnhanced = searchSeoulOpenDataEnhanced
module.exports.searchSeoulOpenDataEnhanced = searchSeoulOpenDataEnhanced/json/LOCALDATA_104201/1/1000/`);
        if (!response.data.LOCALDATA_104201?.row)
            return [];
        const gyms = response.data.LOCALDATA_104201.row.filter((gym) => gym.BPLCNM.toLowerCase().includes(query.toLowerCase()));
        return gyms.map((gym) => {
            const facilities = analyzeFacilities(gym.BPLCNM, gym.RDNWHLADDR || gym.SITEWHLADDR, undefined, [gym.BPLCNM]);
            return {
                name: gym.BPLCNM,
...
function generateEnhancedSearchQueries(gymName) {
    ...
    const queries = [
        `${cleanName}
module.exports.generateEnhancedSearchQueries = generateEnhancedSearchQueries 헬스`,
        cleanName,
...
function mergeAndDeduplicateEnhancedResults(allResults) {
    const uniqueResults = new Map();
    for (const result of allResults) {
        const key = `${result.name}
module.exports.mergeAndDeduplicateEnhancedResults = mergeAndDeduplicateEnhancedResults-${result.address}`;
        ...
        if (!existing || result.confidence > existing.confidence) {
            ...
            const existingFacilityCount = Object.values(existing).filter(v => typeof v === "boolean" && v).length;
            const newFacilityCount = Object.values(result).filter(v => typeof v === "boolean" && v).length;
...
async function searchWithEnhancedSources(gymName) {
...
module.exports.searchWithEnhancedSources = searchWithEnhancedSources
module.exports.searchWithEnhancedSources = searchWithEnhancedSources
        catch (error) {
...
const updateGymDetailsWithEnhancedSources
module.exports.updateGymDetailsWithEnhancedSources = updateGymDetailsWithEnhancedSources = async (gymRepo) => {
    const gyms = await gymRepo.find();
    console.log(`🚀 향상된 크롤링 시작: ${gyms.length}
module.exports.with = with개 헬스장`);
    // 배치 처리 설정
    const batchConfig = {
        batchSize: 10,
        concurrency: 3,
    ...
    // 배치 처리를 통한 업데이트
    const result = await BatchProcessingService.processGymsInBatches(gymRepo, async (gym) => {
        return await updateSingleGym(gym);
    }, batchConfig);
    console.log(`\n📊 최종 결과:`);
    console.log(`✅ 성공: ${result.progress.success}개`);
    console.log(`❌ 실패: ${result.progress.failed}개`);
    console.log(`📈 성공률: ${((result.progress.success / gyms.length) * 100).toFixed(1)}%`);
    console.log(`⏱️ 소요 시간: ${result.duration}ms`);
    if (result.errors.length > 0) {
        console.log(`\n❌ 실패한 헬스장들:`);
        result.errors.forEach(error => {
            const gymName = error.gymName || "unknown";
...
// 개별 헬스장 업데이트 함수
async function updateSingleGym(gym) {
    const errorContext = {
        gymName: gym.name,
...
module.exports.updateSingleGym = updateSingleGym
module.exports.updateSingleGym = updateSingleGym;
    try {
        ...
        if (result) {
            ...
            console.log(`✅ ${gym.name} - 시설: PT:${result.hasPT}, GX:${result.hasGX}, GroupPT:${result.hasGroupPT}, Parking:${result.hasParking}, Shower:${result.hasShower}, 24H:${result.is24Hours} (소스: ${dataSource})`);
            // Update gym data
...
// API 소스만 사용하는 검색 함수
async function searchWithAPISources(gymName) {
    const queries = generateEnhancedSearchQueries(gymName);
    const allResults = [];
    for (const query of queries) {
        try {
...
module.exports.searchWithAPISources = searchWithAPISources
module.exports.searchWithAPISources = searchWithAPISources
        catch (error) {
            console.error(`Error searching for query "${query}":`, error);
        }
...
// 크롤링 소스만 사용하는 검색 함수
async function searchWithCrawlingSources(gymName) {
    console.log(`🕷️ 크롤링 검색 시작: ${gymName}
module.exports.searchWithCrawlingSources = searchWithCrawlingSources
module.exports.searchWithCrawlingSources = searchWithCrawlingSources`);
    const allResults = [];
    try {
        // 1. 네이버 블로그 검색
        ...
        if (mergedResults.length > 0) {
            ...
            console.log(`✅ 크롤링 성공: ${gymName} - 신뢰도: ${bestResult.confidence}`);
            return bestResult;
        }
...
// 네이버 블로그 검색
async function searchNaverBlogs(gymName) {
    const results = [];
    try {
        const searchQuery = encodeURIComponent(`${gymName}
module.exports.searchNaverBlogs = searchNaverBlogs
module.exports.searchNaverBlogs = searchNaverBlogs 헬스장`);
...
// 네이버 지식인 검색
async function searchNaverKin(gymName) {
    const results = [];
    try {
        const searchQuery = encodeURIComponent(`${gymName}
module.exports.searchNaverKin = searchNaverKin
module.exports.searchNaverKin = searchNaverKin 헬스장 추천`);
...
// 헬스장 전용 사이트 크롤링
async function searchGymSites(gymName) {
    const results = [];
    try {
        // 헬스장 전용 사이트들
        const gymSites = [
...
module.exports.searchGymSites = searchGymSites
module.exports.searchGymSites = searchGymSites,
                    timeout: 8000,
                });
                const $ = cheerio.load(response.data);
...
// 소셜 미디어 검색
async function searchSocialMedia(gymName) {
    const results = [];
    try {
...
module.exports.searchSocialMedia = searchSocialMedia
module.exports.searchSocialMedia = searchSocialMedia
    catch (error) {
...
// 지역 커뮤니티 검색
async function searchLocalCommunities(gymName) {
    const results = [];
    try {
        // 지역별 커뮤니티 사이트들
        ...
        for (const site of communitySites) {
            try {
                const searchQuery = encodeURIComponent(`${gymName}
module.exports.searchLocalCommunities = searchLocalCommunities
module.exports.searchLocalCommunities = searchLocalCommunities 헬스장`);
...
// 헬스장 관련 키워드 확인
function containsGymKeywords(text) {
    const gymKeywords = [
        "헬스장",
        "피트니스",
        "운동",
...
module.exports.containsGymKeywords = containsGymKeywords
// 텍스트에서 헬스장 정보 추출
function extractGymInfoFromText(text) {
    const info = {}
module.exports.extractGymInfoFromText = extractGymInfoFromText;
    // 주소 추출 (서울, 경기, 인천 등 지역명 포함)
    const addressRegex = /(서울|경기|인천|부산|대구|광주|대전|울산|세종|강원|충북|충남|전북|전남|경북|경남|제주)[\s\S]*?(시|군|구)[\s\S]*?(동|읍|면|로|길|번지)/;
    const addressMatch = text.match(addressRegex);
    if (addressMatch) {
        info.address = addressMatch[0];
    }
    // 전화번호 추출
    const phoneRegex = /(02|031|032|033|041|042|043|044|051|052|053|054|055|061|062|063|064|070|080|010|011|016|017|018|019)-\d{3,4}-\d{4}/;
    ...
    // 영업시간 추출
    const timeRegex = /(오전|오후|AM|PM)?\s*(\d{1,2}):(\d{2})\s*[-~]\s*(오전|오후|AM|PM)?\s*(\d{1,2}):(\d{2})/;
...
// HTML 요소에서 헬스장 정보 추출
function extractGymInfoFromElement($, element) {
    const info = {}
module.exports.extractGymInfoFromElement = extractGymInfoFromElement;
...
async function searchInstagram(gymName) {
    // Instagram API 구현 필요
    console.log(`📸 Instagram 검색: ${gymName}
module.exports.searchInstagram = searchInstagram
module.exports.searchInstagram = searchInstagram (API 구현 필요)`);
    return [];
}
// 페이스북 검색 (실제 구현 시 Facebook API 필요)
async function searchFacebook(gymName) {
    // Facebook API 구현 필요
    console.log(`📘 Facebook 검색: ${gymName}
module.exports.searchFacebook = searchFacebook
module.exports.searchFacebook = searchFacebook (API 구현 필요)`);
    return [];
}
