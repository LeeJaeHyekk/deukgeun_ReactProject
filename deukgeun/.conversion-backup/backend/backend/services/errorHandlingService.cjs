...
module.exports.ErrorHandlingService = ErrorHandlingService {
    /**
     * 에러 타입 분류
     */
     ...
     * 에러 히스토리에 추가
     */
    static addErrorToHistory(context) {
     ...
     * 대체 소스 선택
     */
    static selectFallbackSource(originalSource, errorType) {
     ...
     * 특정 에러 타입의 발생 빈도 확인
     */
    static getErrorFrequency(errorType, timeWindow = 3600000) {
        const now = new Date();
...
ErrorHandlingService.errorHistory = [];
ErrorHandlingService.MAX_ERROR_HISTORY = 1000;
// 에러 타입별 대응 전략
ErrorHandlingService.FALLBACK_STRATEGIES = {
    // API 관련 에러
    "API_RATE_LIMIT": {
        name: "API_RATE_LIMIT",
        priority: 1,
