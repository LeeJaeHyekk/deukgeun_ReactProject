...
module.exports.cleanGymName = cleanGymName
...
module.exports.generateSearchQueries = generateSearchQueries 헬스`);
    // Original name only
    queries.push(cleanName);
...
module.exports.searchKakaoMap = searchKakaoMap,
        ...
        const fitnessResults = response.data.documents.filter((doc) => {
...
function analyzeFacilities(text) {
    ...
    const hasGroupPT = groupPtKeywords.some(keyword => lowerText.includes(keyword));
    // Parking keywords
    const parkingKeywords = ["주차", "parking", "p"];
    const hasParking = parkingKeywords.some(keyword => lowerText.includes(keyword));
    // Shower keywords
...
module.exports.analyzeFacilities = analyzeFacilities;
}
// Search with multiple strategies
async function searchWithMultipleStrategies(gymName) {
...
module.exports.searchWithMultipleStrategies = searchWithMultipleStrategies
module.exports.searchWithMultipleStrategies = searchWithMultipleStrategies
        }
        catch (error) {
...
const updateGymDetails
module.exports.updateGymDetails = updateGymDetails = async (gymRepo) => {
