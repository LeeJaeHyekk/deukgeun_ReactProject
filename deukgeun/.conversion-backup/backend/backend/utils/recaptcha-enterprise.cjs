 ...
 */
const verifyRecaptchaEnterprise
module.exports.verifyRecaptchaEnterprise = verifyRecaptchaEnterprise = async (token, expectedAction) => {
    try {
        ...
        const response = await axios.post(`https://recaptchaenterprise.googleapis.com/v1/projects/${projectId}/assessments?key=${apiKey}`, requestData, {
            headers: {
 ...
 */
const processRecaptchaScore
module.exports.processRecaptchaScore = processRecaptchaScore = (score, action) => {
 ...
 */
const recaptchaEnterpriseMiddleware
module.exports.recaptchaEnterpriseMiddleware = recaptchaEnterpriseMiddleware = (action, minScore = 0.5) => {
 ...
 */
const checkRecaptchaEnterpriseHealth
module.exports.checkRecaptchaEnterpriseHealth = checkRecaptchaEnterpriseHealth = async () => {
    try {
        const projectId = process.env.RECAPTCHA_PROJECT_ID;
        const apiKey = process.env.RECAPTCHA_API_KEY;
        if (!projectId || !apiKey) {
        ...
        // 간단한 API 연결 테스트
        const response = await axios.get(`https://recaptchaenterprise.googleapis.com/v1/projects/${projectId}?key=${apiKey}`, { timeout: 5000 });
        return {
            status: 'healthy',
