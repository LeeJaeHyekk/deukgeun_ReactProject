...
module.exports.verifyRecaptcha = verifyRecaptcha
        ...
        if (!secret || secret === "") {
            ...
            if (process.env.NODE_ENV === "development") {
                logger.warn("개발 환경에서 reCAPTCHA 시크릿 키가 설정되지 않았지만 더미 토큰 허용");
                return true;
            }
...
module.exports.validateRecaptchaConfig = validateRecaptchaConfig() {
    const secret = process.env.RECAPTCHA_SECRET_KEY || process.env.RECAPTCHA_SECRET;
    if (!secret || secret === "") {
        if (process.env.NODE_ENV === "development") {
            logger.warn("개발 환경: reCAPTCHA 시크릿 키가 설정되지 않음 (더미 토큰 사용)");
            return true;
        }
module.exports.validateRecaptchaConfig = validateRecaptchaConfig
