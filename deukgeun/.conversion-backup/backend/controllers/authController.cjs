...
async function login(req, res) {
    try {
        ...
        try {
            logger_1.logger.info(`reCAPTCHA 검증 시작 - IP: ${req.ip}, Email: ${email}`);
            const isHuman = await (0, recaptcha_1.verifyRecaptcha)(recaptchaToken, "LOGIN", req);
            if (!isHuman) {
            ...
            logger_1.logger.info(`reCAPTCHA 검증 통과 - IP: ${req.ip}, Email: ${email}`);
        }
        catch (recaptchaError) {
            logger_1.logger.error(`reCAPTCHA 검증 오류 - IP: ${req.ip}, Email: ${email}`, recaptchaError);
            if (process.env.NODE_ENV === 'development') {
                logger_1.logger.warn('개발 환경: reCAPTCHA 검증 오류를 무시하고 계속 진행합니다.');
            }
            else {
                res.status(503).json({
                    success: false,
        ...
        logger_1.logger.info(`사용자 찾음 - User ID: ${user.id}, Email: ${email}`);
        logger_1.logger.info(`비밀번호 비교 시작 - User ID: ${user.id}`);
        const isPasswordValid = await bcrypt_1.default.compare(password, user.password);
        if (!isPasswordValid) {
            logger_1.logger.warn(`로그인 실패 - 비밀번호 불일치 - IP: ${req.ip}, Email: ${email}, User ID: ${user.id}`);
            res.status(401).json({
                success: false,
                message: "이메일 또는 비밀번호가 틀렸습니다.",
                error: "인증 실패",
        ...
        logger_1.logger.info(`비밀번호 검증 통과 - User ID: ${user.id}`);
        const { accessToken, refreshToken } = (0, jwt_1.createTokens)(user.id, user.role);
        const refreshHash = await (0, jwt_1.hashRefreshToken)(refreshToken);
        ...
        const sameSite = (isProduction && isSecure && isHttps) ? "none" : "lax";
        logger_1.logger.debug("쿠키 설정", {
            isProduction,
            isSecure,
            isHttps,
            sameSite,
        ...
        res
            ...
            data: {
                accessToken,
                user: userTransformer_1.UserTransformer.toDTO(user),
            }
        });
    }
    catch (error) {
...
async function refreshToken(req, res) {
    try {
        ...
        const isValid = await (0, jwt_1.compareRefreshToken)(token, user.refreshTokenHash);
        if (!isValid) {
            logger_1.logger.warn(`Refresh token 불일치 - User ID: ${user.id}, IP: ${req.ip}`);
            res.clearCookie("refreshToken", { path: "/" });
            res.status(401).json({
                success: false,
                message: "리프레시 토큰 불일치",
        ...
        const newRefreshHash = await (0, jwt_1.hashRefreshToken)(newRefreshToken);
        ...
        const isHttps = origin?.startsWith('https://') || req.protocol === 'https';
        const sameSite = (isProduction && isSecure && isHttps) ? "none" : "lax";
        logger_1.logger.debug("쿠키 설정 (refreshToken)", {
            isProduction,
            isSecure,
            isHttps,
            sameSite,
            ...
            try {
                ...
                if (payload) {
                    ...
                    if (user) {
                        ...
                        await userRepo.save(user);
                        logger_1.logger.info(`로그아웃 - User ID: ${user.id}, refresh token 해시 제거`);
                    }
                }
            }
            catch (error) {
                logger_1.logger.warn("로그아웃 시 refresh token 검증 실패:", error);
            }
        }
...
const register = async (req, res) => {
    try {
        ...
        if (!recaptchaToken || typeof recaptchaToken !== 'string' || recaptchaToken.trim() === '') {
            ...
            logger_1.logger.warn(`회원가입 reCAPTCHA 토큰 누락 - IP: ${req.ip}, Email: ${email}`);
            res.status(400).json({
                success: false,
        ...
        const isHuman = await (0, recaptcha_1.verifyRecaptcha)(recaptchaToken, "REGISTER", req);
        if (!isHuman) {
        ...
        const responseData = {
            ...
            accessToken,
            refreshToken,
            user: userTransformer_1.UserTransformer.toDTO(newUser),
        };
        console.log("📤 응답 데이터:", {
        ...
        const isHttps = origin?.startsWith('https://') || req.protocol === 'https';
        const sameSite = (isProduction && isSecure && isHttps) ? "none" : "lax";
        logger_1.logger.debug("쿠키 설정 (register)", {
            isProduction,
            isSecure,
            isHttps,
            sameSite,
...
async function findId(req, res) {
    try {
        ...
        const isHuman = await (0, recaptcha_1.verifyRecaptcha)(recaptchaToken, "FIND_ACCOUNT", req);
        if (!isHuman) {
...
async function findPassword(req, res) {
    try {
        ...
        const isHuman = await (0, recaptcha_1.verifyRecaptcha)(recaptchaToken, "FIND_PASSWORD", req);
        if (!isHuman) {
...
async function resetPasswordSimpleStep1(req, res) {
    try {
        ...
        const recaptchaValid = await (0, recaptcha_1.verifyRecaptcha)(recaptchaToken, "FIND_ACCOUNT", req);
        if (!recaptchaValid) {
            res.status(400).json({
                success: false,
                message: "reCAPTCHA 검증에 실패했습니다.",
...
async function updateProfile(req, res) {
    try {
        ...
        if (newPassword && newPassword.trim()) {
            ...
            const isPasswordValid = await bcrypt_1.default.compare(currentPassword.trim(), user.password);
            if (!isPasswordValid) {
                logger_1.logger.warn(`회원정보 수정 실패 - 비밀번호 불일치: User ID: ${user.id}`);
                res.status(401).json({
                    success: false,
            ...
            const saltRounds = 10;
            user.password = await bcrypt_1.default.hash(trimmedNewPassword, saltRounds);
            logger_1.logger.info(`비밀번호 변경 성공 - User ID: ${user.id}`);
        }
        user.updatedAt = new Date();
        await userRepo.save(user);
        logger_1.logger.info(`회원정보 수정 성공 - User ID: ${user.id}`);
        res.json({
            success: true,
        ...
        logger_1.logger.error(`회원정보 수정 실패: ${error.message}`, error);
        res.status(500).json({
            success: false,
