...
class CommentController {
    constructor() {
        this.createComment = async (req, res) => {
            try {
                ...
                if (!userId || isNaN(userId)) {
                    console.log("인증 실패: userId 없음 또는 유효하지 않음", { userId: req.user?.userId });
                ...
                const repo = databaseConfig_1.AppDataSource.getRepository(Comment_1.Comment);
                ...
                try {
                    await this.levelService.grantExp(userId, "comment", "comment_creation", { commentId: savedComment.id, postId: savedComment.postId });
                }
                catch (levelError) {
                    console.error("경험치 부여 실패:", levelError);
                }
                const commentDTO = (0, transformers_1.toCommentDTO)(savedComment);
                res.status(201).json({
                    success: true,
        ...
        this.getCommentsByPostId = async (req, res) => {
            try {
                const { id: postId } = req.params;
                const repo = databaseConfig_1.AppDataSource.getRepository(Comment_1.Comment);
                ...
                const commentDTOs = (0, transformers_1.toCommentDTOList)(comments);
                res.json({
                    success: true,
                    data: commentDTOs,
        ...
        this.updateComment = async (req, res) => {
            try {
                ...
                console.log("사용자 정보 userId:", req.user?.userId);
                console.log("사용자 정보 userId 타입:", typeof req.user?.userId);
                const { id } = req.params;
                const { content } = req.body;
                const userId = Number(req.user?.userId);
                if (!userId || isNaN(userId)) {
                ...
                const commentDTO = (0, transformers_1.toCommentDTO)(updatedComment);
                res.json({
                    success: true,
