...
class EmailService {
    ...
    initializeTransporter() {
        ...
        if (emailConfig) {
            ...
            logger_1.logger.warn("Email service not configured - using mock mode");
            this.isConfigured = false;
        }
    }
    getEmailConfig() {
    ...
    async sendEmail(emailContent) {
        try {
            if (!this.isConfigured || !this.transporter) {
                logger_1.logger.info(`[MOCK EMAIL] To: ${emailContent.to}, Subject: ${emailContent.subject}`);
                logger_1.logger.info(`[MOCK EMAIL] Content: ${emailContent.text || emailContent.html}`);
                return true;
            }
            ...
            logger_1.logger.info(`Email sent successfully to ${emailContent.to}: ${result.messageId}`);
            return true;
        }
        catch (error) {
    ...
    async sendVerificationCode(email, code, type, nickname) {
        const subject = type === "find_id"
            ? "[GymApp] 아이디 찾기 인증 코드"
    ...
    async sendPasswordResetLink(email, resetToken, nickname) {
    ...
    async sendIdFoundEmail(email, nickname) {
    ...
    generateVerificationEmailHTML(code, type, nickname) {
    ...
    generateVerificationEmailText(code, type, nickname) {
        const action = type === "find_id" ? "아이디 찾기" : "비밀번호 재설정";
        return `
GymApp ${action} 인증 코드
...
© 2024 GymApp. All rights reserved.
    ...
    generatePasswordResetEmailHTML(resetUrl, nickname) {
        return `
      <!DOCTYPE html>
      <html>
      <head>
    ...
    generatePasswordResetEmailText(resetUrl, nickname) {
        return `
GymApp 비밀번호 재설정
...
© 2024 GymApp. All rights reserved.
    ...
    generateIdFoundEmailHTML(email, nickname) {
        return `
      <!DOCTYPE html>
      <html>
      <head>
    ...
    generateIdFoundEmailText(email, nickname) {
        return `
GymApp 아이디 찾기 결과
