...
const { validateFrontendEnvVars  } = require('../utils/envValidator')
...
const getEnvConfig = () => {
  ...
  if (isDevelopment) {
    return {
      ...
      // Database URL
      DATABASE_URL: process.env.VITE_DATABASE_URL || 'mysql://localhost:3306/deukgeun',
      
      // Redis URL
      REDIS_URL: process.env.VITE_REDIS_URL || 'redis://localhost:6379',
      
      // JWT Secret
      JWT_SECRET: process.env.VITE_JWT_SECRET || 'dev-secret-key',
      ...
      // Logging
      LOG_LEVEL: process.env.VITE_LOG_LEVEL || 'debug',
  ...
  if (isProduction) {
    return {
      ...
      // Email Configuration
      SMTP_HOST: process.env.VITE_SMTP_HOST || 'smtp.gmail.com',
      SMTP_PORT: process.env.VITE_SMTP_PORT || '587',
      SMTP_USER: process.env.VITE_SMTP_USER || '',
      SMTP_PASS: process.env.VITE_SMTP_PASS || '',
  ...
  return {
    ...
    // JWT Secret
    JWT_SECRET: process.env.VITE_JWT_SECRET || 'test-secret-key',
...
const ENV_CONFIG
module.exports.ENV_CONFIG = ENV_CONFIG = getEnvConfig()
...
module.exports.IMAGE_MATCHING_CONFIG = IMAGE_MATCHING_CONFIG = {
  MACHINE_IMAGES: {
