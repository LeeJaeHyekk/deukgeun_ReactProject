...
const { isString, isNumber, isBoolean, isObject, isArray, isDateString  } = require('../types/guards')
...
module.exports.TypedApiClient = TypedApiClient {
...
module.exports.isValidUser = isValidUser(value: unknown): value is User {
...
module.exports.isValidMachine = isValidMachine(value: unknown): value is Machine {
  if (!isObject(value)) return false
...
module.exports.isValidWorkoutPlan = isValidWorkoutPlan(value: unknown): value is WorkoutPlan {
  if (!isObject(value)) return false
...
// 운동 목표 타입 검증
function isValidWorkoutGoal
module.exports.isValidWorkoutGoal = isValidWorkoutGoal(value: unknown): value is WorkoutGoal {
  if (!isObject(value)) return false
...
module.exports.isValidArray = isValidArray<T>(
  value: unknown, 
...
module.exports.isValidUserArray = isValidUserArray(value: unknown): value is User[] {
  return isValidArray(value, isValidUser)
}

// 머신 배열 검증
function isValidMachineArray
module.exports.isValidMachineArray = isValidMachineArray(value: unknown): value is Machine[] {
  return isValidArray(value, isValidMachine)
}

// 운동 계획 배열 검증
function isValidWorkoutPlanArray
module.exports.isValidWorkoutPlanArray = isValidWorkoutPlanArray(value: unknown): value is WorkoutPlan[] {
  return isValidArray(value, isValidWorkoutPlan)
}

// 운동 목표 배열 검증
function isValidWorkoutGoalArray
module.exports.isValidWorkoutGoalArray = isValidWorkoutGoalArray(value: unknown): value is WorkoutGoal[] {
  return isValidArray(value, isValidWorkoutGoal)
}

function isValidPaginatedResponse
module.exports.isValidPaginatedResponse = isValidPaginatedResponse<T>(value: unknown): value is ApiPaginatedResponse<T> {
...
module.exports.parsePaginatedResponse = parsePaginatedResponse<T>(
  response: unknown,
  itemValidator?: (item: unknown) => item is T
...
module.exports.handleApiError = handleApiError(error: unknown, context: string): never {
  if (error instanceof Error) {
...
module.exports.isValidUserData = isValidUserData(value: unknown): value is {
  id: number
  nickname: string
  email: string
  createdAt: string
...
function isValidMachineData
module.exports.isValidMachineData = isValidMachineData(value: unknown): value is {
  id: number
  name: string
  category: string
  difficulty: string
...
function isValidWorkoutPlanData
module.exports.isValidWorkoutPlanData = isValidWorkoutPlanData(value: unknown): value is {
  id: number
  name: string
  description?: string
