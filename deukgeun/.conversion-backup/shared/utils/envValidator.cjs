...
module.exports.validateRequiredEnvVars = validateRequiredEnvVars(): RequiredEnvVars {
  ...
  if (isBrowserEnvironment()) {
    ...
    requiredVars = {
      ...
      CORS_ORIGIN: process.env.VITE_CORS_ORIGIN || '',
    }
...
module.exports.validateOptionalEnvVars = validateOptionalEnvVars(): OptionalEnvVars {
  if (isBrowserEnvironment()) {
    // 브라우저 환경 (프론트엔드)
    return {
      ...
      DB_PASSWORD: process.env.VITE_DB_PASSWORD,
    }
  } else if (isNodeEnvironment()) {
    // Node.js 환경 (백엔드)
    return {
...
// 환경 변수 검증 및 경고
function validateAndWarnEnvVars
module.exports.validateAndWarnEnvVars = validateAndWarnEnvVars(): void {
  try {
...
function validateFrontendEnvVars
module.exports.validateFrontendEnvVars = validateFrontendEnvVars(): void {
  if (!isBrowserEnvironment()) {
    return
  }
...
function validateBackendEnvVars
module.exports.validateBackendEnvVars = validateBackendEnvVars(): void {
...
module.exports.validateUrl = validateUrl(url: string, name: string): void {
  try {
    new URL(url)
  } catch {
...
module.exports.validateEnvironmentUrls = validateEnvironmentUrls(): void {
...
module.exports.validateDevEnvironment = validateDevEnvironment(): void {
...
module.exports.validateProductionEnvironment = validateProductionEnvironment(): void {
...
module.exports.runEnvironmentValidation = runEnvironmentValidation(): void {
  try {
    if (isBrowserEnvironment()) {
      // 브라우저 환경 (프론트엔드)
