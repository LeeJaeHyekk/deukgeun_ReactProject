...
class UnifiedRunner {
  ...
  private async runConversion(): Promise<{ success: boolean; results: any }> {
    try {
      ...
      // 변환 스크립트 실행
      const convertScript = path.join(this.options.projectRoot, 'scripts', 'convert.ts')
      if (fs.existsSync(convertScript)) {
        execSync(`npx ts-node ${convertScript} --verbose`, {
          stdio: this.options.verbose ? 'inherit' : 'pipe',
          cwd: this.options.projectRoot,
          timeout: this.options.timeout
        })
      } else {
        logWarning('변환 스크립트를 찾을 수 없습니다. 건너뜁니다.')
      }

      logSuccess('코드 변환 완료')
      return { success: true, results: { converted: true } }
  ...
  private async runDeploy(): Promise<{ success: boolean; results: any }> {
    try {
      ...
      if (fs.existsSync(deployScript)) {
        execSync(`npx ts-node ${deployScript} --verbose`, {
          stdio: this.options.verbose ? 'inherit' : 'pipe',
          cwd: this.options.projectRoot,
          timeout: this.options.timeout
        })
