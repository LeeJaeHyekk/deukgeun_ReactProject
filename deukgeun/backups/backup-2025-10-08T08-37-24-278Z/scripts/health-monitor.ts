...
// 시스템 리소스 모니터링
function getSystemResources(): SystemResources | null {
  try {
    // CPU 사용률 계산
    const cpus = os.cpus()
    let totalIdle = 0
...
// 애플리케이션 헬스체크
function checkApplicationHealth(): ApplicationHealth[] {
  const endpoints = [
    { name: 'Backend API', url: 'http://localhost:5000/health' },
    { name: 'Frontend', url: 'http://localhost:3000' }
  ]
  
  const results: ApplicationHealth[] = []
...
// 알림 생성
function generateAlert(type: string, value: number, threshold: number): Alert {
  const alert: Alert = {
...
// 로그 파일에 기록
function writeToLogFile(data: MonitoringData): void {
  try {
    ...
    logError(`로그 파일 기록 실패: ${(error as Error).message}`)
  }
}
...
// 모니터링 리포트 생성
function generateReport(data: MonitoringData): Report {
  const report: Report = {
    timestamp: new Date().toISOString(),
    summary: {
...
function runMonitoring(): void {
  ...
  if (alerts.length > 0) {
    ...
    alerts.forEach(alert => {
      ...
      log(`${alert.severity.toUpperCase()}: ${alert.type} - ${alert.message || `${alert.value}% (임계값: ${alert.threshold}%)`}`, severity as ColorKey)
    })
  } else {
...
<body>
    ...
    logError(`대시보드 생성 실패: ${(error as Error).message}`)
  }
}
...
function main(): void {
  ...
  switch (command) {
    ...
    default:
      logInfo('사용법:')
      logInfo('  node health-monitor.ts monitor  - 지속적인 모니터링')
      logInfo('  node health-monitor.ts check    - 단일 체크')
      logInfo('  node health-monitor.ts dashboard - 대시보드 생성')
  }
}
