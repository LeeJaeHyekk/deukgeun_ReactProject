...
async function testServerConnection(): Promise<boolean> {
  ...
  try {
    ...
    logError(`서버 연결 실패: ${(error as Error).message}`)
    return false
  }
}

// reCAPTCHA 설정 확인
async function testRecaptchaConfig(): Promise<boolean> {
  ...
  try {
    ...
    log(`  - 사이트 키: ${configData.siteKey ? '설정됨' : '설정되지 않음'}`)
    log(`  - 프로젝트 ID: ${configData.projectId || '설정되지 않음'}`)
    log(`  - API 키: ${configData.hasApiKey ? '설정됨' : '설정되지 않음'}`)
    log(`  - 시크릿 키: ${configData.hasSecret ? '설정됨' : '설정되지 않음'}`)
    
    if (configData.siteKey && configData.projectId && configData.hasApiKey) {
    ...
    logError(`reCAPTCHA 설정 확인 실패: ${(error as Error).message}`)
    return false
  }
}

// reCAPTCHA 헬스체크
async function testRecaptchaHealth(): Promise<boolean> {
  ...
  try {
    ...
    logError(`reCAPTCHA 헬스체크 실패: ${(error as Error).message}`)
    return false
  }
}

// 가짜 토큰으로 검증 테스트
async function testRecaptchaVerification(): Promise<boolean> {
  ...
  try {
    ...
    if ((error as any).response && (error as any).response.status === 400) {
      ...
      logError(`reCAPTCHA 검증 테스트 실패: ${(error as Error).message}`)
      return false
    }
  }
}
...
// 브라우저 테스트 안내
function showBrowserTestInstructions(): void {
  logInfo('🌐 브라우저 테스트 안내:')
  log('')
...
if (require.main === module) {
  runTests().catch(error => {
    logError(`테스트 실행 중 오류 발생: ${(error as Error).message}`)
    process.exit(1)
  })
}
