...
// 기본 옵션
const defaultOptions: DeployScriptOptions = {
 ...
 * 배포 스크립트 실행
 */
export async function runDeployScript(options: Partial<DeployScriptOptions> = {}): Promise<{ success: boolean; duration: number; results: any }> {
  ...
  try {
    ...
    // 배포 실행
    const deployResult = await executeDeploy(finalOptions.projectRoot, {
      ...
      autoRecovery: finalOptions.autoRecovery,
      safety: finalOptions.safety
    })

    if (deployResult.success) {
      printDeployStats(deployResult.results)
    }
    ...
    logSeparator('=', 60, 'red')
    logError(`배포 스크립트 실패: ${error.message}`)
    logSeparator('=', 60, 'red')
 ...
 * 실행 계획 출력
 */
function printExecutionPlan(options: DeployScriptOptions): void {
  logInfo('\n📋 실행 계획:')
 ...
 * 명령행 인수 파싱
 */
function parseArguments(): Partial<DeployScriptOptions> {
  const args = process.argv.slice(2)
  const options: Partial<DeployScriptOptions> = {}
...
예시:
  node deploy-script.ts --verbose
  node deploy-script.ts --no-backup --parallel
  node deploy-script.ts --dry-run
`)
}
...
async function main(): Promise<void> {
  try {
    ...
    const result = await runDeployScript(options)
...
// export { runDeployScript } - 이미 위에서 export됨
