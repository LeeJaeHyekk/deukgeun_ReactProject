...
function checkWindowsEnvironment() {
  ...
  if (!isWindows) {
    ...
    log('Linux/Mac 환경에서는 deploy-optimized.cjs를 사용하세요.', 'yellow')
  }
...
// 의존성 확인 (Windows)
function checkDependenciesWindows() {
...
// 환경 변수 설정 (Windows)
function setupEnvironmentWindows() {
...
// PM2 설정 확인 (Windows)
function validatePM2ConfigWindows() {
...
// 기존 서비스 정리 (Windows)
function cleanupServicesWindows() {
...
// Windows 성능 최적화
function optimizeWindowsPerformance() {
...
// 배포 정보 출력 (Windows)
function printWindowsDeploymentInfo() {
