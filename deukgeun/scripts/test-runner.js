...
class TestLogger {
  ...
  // 로그를 파일로 저장
  saveLogs(filename = 'test-results.json') {
...
class TestRunner {
  ...
  async runTest(type) {
    ...
    try {
      ...
      this.logger.error(`${option.description} 실패:`, error.message)
      this.results[type] = {
        success: false,
        error: error.message,
  ...
  async runCoverage() {
    ...
    try {
      ...
      this.logger.error('코드 커버리지 테스트 실패:', error.message)
      return false
    }
  }
            ...
            <div class="test-suite">
                ...
                ${
                  suite.tests
                    ? `
                    <ul>
                        ${suite.tests.map(test => `<li>${test.name}: ${test.status}</li>`).join('')}
                    </ul>
                `
                    : ''
                }
...
async function main() {
  ...
  try {
    ...
    runner.logger.error('테스트 실행 중 오류 발생:', error.message)
    process.exit(1)
  }
}
