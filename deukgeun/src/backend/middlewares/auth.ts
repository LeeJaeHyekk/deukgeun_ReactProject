...
export const authMiddleware = (
  ...
  console.log(
    `🔐 [AuthMiddleware:${requestId}] 요청 URL: ${req.method} ${req.url}`
  )

  try {
    const authHeader = req.headers["authorization"]
    console.log(
      `🔐 [AuthMiddleware:${requestId}] Authorization 헤더:`,
    ...
    if (req.params.userId) {
      ...
      if (payload.userId !== requestedUserId && payload.role !== "admin") {
        console.log(
          `🔐 [AuthMiddleware:${requestId}] 권한 없음 - 사용자 ID 불일치`
        )
        console.log(
          `🔐 [AuthMiddleware:${requestId}] 요청된 사용자 ID: ${requestedUserId}`
        )
        console.log(
          `🔐 [AuthMiddleware:${requestId}] 토큰의 사용자 ID: ${payload.userId}`
        )
        console.log(
          `🔐 [AuthMiddleware:${requestId}] 사용자 역할: ${payload.role}`
        )
        return res.status(403).json({
    ...
    req.user = payload
    console.log(
      `🔐 [AuthMiddleware:${requestId}] 인증 성공, 사용자 정보 설정:`,
      req.user
    )
    next()
  } catch (error) {
