...
async function testSecurity() {
  ...
  try {
    ...
    if (error.response?.status === 400) {
      ...
      console.log("❌ 예상과 다른 에러:", error.response?.status, error.response?.data);
    }
  }
  ...
  try {
    ...
    console.log("❌ XSS 방지 테스트 실패:", error.response?.data?.message || error.message);
  }
  ...
  try {
    ...
    if (error.response?.status === 500 && error.response?.data?.error?.includes("이미 존재합니다")) {
  ...
  try {
    ...
    console.log("❌ 보안 헤더 테스트 실패:", error.response?.status, error.response?.data);
  }
