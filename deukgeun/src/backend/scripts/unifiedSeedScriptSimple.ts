// ============================================================================
// 통합 DB Seed 스크립트 - EC2 배포용 (간소화 버전)
// ============================================================================
...
async function seedMachines(): Promise<void> {
  ...
  try {
    ...
    // SQL 직접 실행으로 성능 최적화
    const insertPromises = machineData.map((machine: any) => {
      const values = [
        machine.id,
        `'${machine.machineKey}'`,
    ...
    logger.info(`✅ 총 ${machineData.length}개 기구 데이터 시드 완료`)
...
async function seedUsers(): Promise<any[]> {
  ...
  try {
    ...
    for (const userData of sampleUsers) {
      ...
      logger.info(`✅ 사용자 생성 완료: ${userData.nickname}`)
    }
...
async function seedGyms(): Promise<void> {
  ...
  try {
    for (const gymData of sampleGyms) {
      ...
      logger.info(`✅ 헬스장 생성 완료: ${gymData.name}`)
    }

    logger.info(`✅ 총 ${sampleGyms.length}개 헬스장 데이터 시드 완료`)
...
async function seedHomePageConfigs(): Promise<void> {
  ...
  try {
    ...
    logger.info(`✅ 총 ${defaultConfigs.length}개 홈페이지 설정 시드 완료`)
...
async function seedCommunityData(users: any[]): Promise<void> {
  ...
  try {
    for (const postData of samplePosts) {
      ...
      logger.info(`✅ 게시글 생성 완료: ${postData.title}`)
