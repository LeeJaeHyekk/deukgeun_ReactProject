...
async function searchSeoulOpenData(query: string): Promise<SearchResult[]> {
  try {
    ...
    if (!response.data.LOCALDATA_104201 || !response.data.LOCALDATA_104201.row) {
      return [];
    }
    ...
    console.warn(`⚠️ 서울시 공공데이터 검색 실패: ${query} - ${(error as Error).message}`);
    return [];
  }
}
...
async function searchNaverBlog(query: string): Promise<SearchResult[]> {
  try {
    ...
    console.warn(`⚠️ 네이버 블로그 검색 실패: ${query} - ${(error as Error).message}`);
    return [];
  }
}
...
async function searchInstagramHashtag(query: string): Promise<SearchResult[]> {
  try {
    ...
    console.warn(`⚠️ 인스타그램 검색 실패: ${query} - ${(error as Error).message}`);
    return [];
  }
}
...
async function searchFacebookPage(query: string): Promise<SearchResult[]> {
  try {
    ...
    console.warn(`⚠️ 페이스북 검색 실패: ${query} - ${(error as Error).message}`);
    return [];
  }
}
...
async function searchGymDirectory(query: string): Promise<SearchResult[]> {
  try {
    ...
    for (const site of directorySites) {
      try {
        const response = await axios.get(`${site}/search?q=${encodeURIComponent(query)}`, {
          headers: {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
          },
          timeout: 5000
        ...
        console.warn(`⚠️ 디렉토리 사이트 크롤링 실패: ${site} - ${(siteError as Error).message}`);
      }
    }

    return results;
  } catch (error) {
    console.warn(`⚠️ 헬스장 디렉토리 검색 실패: ${query} - ${(error as Error).message}`);
    return [];
  }
}

// 좌표 기반 역검색 (주소로 좌표 찾기)
async function reverseGeocodeAddress(address: string): Promise<{ latitude: number; longitude: number } | null> {
  try {
...
// 고급 멀티 소스 검색
export async function searchWithAdvancedSources(gymName: string): Promise<SearchResult | null> {
  const searchQueries = generateAdvancedSearchQueries(gymName);
  const allResults: SearchResult[] = [];
  
  console.log(`🔍 [고급멀티소스] ${gymName} 검색 시작 (${searchQueries.length}개 검색어)`);
...
// 고급 헬스장 정보 업데이트 함수
export const updateGymDetailsWithAdvancedSources = async (gymRepo: Repository<Gym>) => {
  const gyms = await gymRepo.find();
  console.log(`📊 총 ${gyms.length}개의 헬스장 데이터를 고급 멀티소스로 크롤링합니다.`);

  let successCount = 0;
  let errorCount = 0;
  let failedGyms: string[] = [];
  ...
  for (let i = 0; i < gyms.length; i++) {
    ...
    try {
      ...
      console.warn(`⚠️ [오류] ${gym.name} 처리 중 문제 발생: ${(error as Error).message}`);
      failedGyms.push(gym.name);
      errorCount++;
    }
