...
export class SectionGenerationService {
  ...
  generateSectionsFromGoal(
    ...
    try {
      ...
      const config = (DIFFICULTY_CONFIG as any)[goalConfig.intensity]
      ...
      exercises.forEach((exerciseName, index) => {
        ...
        const section: AutoGeneratedSection = {
          id: (Date.now() + index).toString(), // 임시 ID 생성
          name: exerciseName,
          exerciseName,
          exercises: [],
          goalId:
          ...
          difficulty: goalConfig.intensity,
        }
  ...
  private getExercisesForGoal(goalConfig: GoalSectionConfig): string[] {
    ...
    const exercisesForGoal =
      (GOAL_TYPE_EXERCISES as any)[goalConfig.goalType]?.[
        muscleGroup || "chest"
      ] || []
    ...
    return exercisesForGoal.slice(0, exerciseCount)
  }
  ...
  calculateNextWeight(
    ...
    const config = (DIFFICULTY_CONFIG as any)[
      intensity as keyof typeof DIFFICULTY_CONFIG
    ]
