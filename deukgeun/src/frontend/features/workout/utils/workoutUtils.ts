...
export function updateGoalValue(
  ...
  switch (goalType) {
    case "weight":
      // 무게 목표는 섹션 완료 시 증가
      return currentValue + (completedSection.weight || 0)
    case "reps":
      // 반복 횟수는 섹션의 총 반복 횟수 추가
      return currentValue + completedSection.sets * completedSection.reps
    case "duration":
      // 운동 시간은 섹션 완료 시간 추가
      return currentValue + (completedSection.restTime || 0)
    case "frequency":
      // 빈도는 완료된 섹션 수
...
export function calculateSectionDuration(
  ...
  // 각 세트당 평균 운동 시간 (초)
  const exerciseTimePerSet = reps * 3 // 1회당 3초 가정

  // 총 운동 시간 + 휴식 시간
  const totalExerciseTime = sets * exerciseTimePerSet
  const totalRestTime = (sets - 1) * restTime // 마지막 세트 후에는 휴식 없음
...
export function calculateSectionExperience(
  ...
  const baseExp = section.sets * section.reps
...
export function groupSectionsByMuscleGroup(
  ...
  return sections.reduce(
    (groups, section) => {
      ...
      const muscleGroup = extractMuscleGroupFromExercise(section.exerciseName)
  ...
  return {
    ...
    completionRate:
      totalSections > 0
        ...
        : 0,
    totalSets: completedSections.reduce((sum, s) => sum + s.sets, 0),
    totalReps: completedSections.reduce((sum, s) => sum + s.sets * s.reps, 0),
    totalWeight: completedSections.reduce((sum, s) => sum + (s.weight || 0), 0),
    averageProgress:
      completedSections.length > 0
        ? Math.round(
            completedSections.reduce((sum, s) => sum + s.progress, 0) /
