...
export const handlers = [
  ...
  // 머신 관련 API
  http.get(API_ENDPOINTS.MACHINES.LIST, () => {
    return HttpResponse.json({
      success: true,
      message: '머신 목록 조회 성공',
      data: {
  ...
  http.get(`${API_ENDPOINTS.MACHINES.GET_BY_ID(1)}`, ({ params }) => {
  ...
  // 커뮤니티 관련 API
  http.get(API_ENDPOINTS.POSTS.LIST, () => {
    return HttpResponse.json({
      success: true,
  ...
  http.get(`${API_ENDPOINTS.POSTS.GET_BY_ID(1)}`, ({ params }) => {
  ...
  // 헬스장 API
  http.get(API_ENDPOINTS.GYMS.LIST, () => {
    return HttpResponse.json({
      success: true,
