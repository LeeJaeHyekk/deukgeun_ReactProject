...
import { LevelConfig } from "../types/dto"
...
export class LevelService {
  ...
  private calculateRequiredExp(level: number): number {
    ...
    return Math.floor(
      config.baseExp *
        Math.pow(config.multiplier, level - 1)
    )
  }
  ...
  private async checkCooldown(
    ...
    try {
      ...
      levelUpFormula: (level: number) => level * 100,
      cooldownTimes: { workout: 3600000, post: 1800000 },
      levelRewards: { 1: { badge: "beginner" }, 10: { badge: "expert" } },
      dailyExpLimit: 1000,
      expValues: { workout: 50, post: 20, comment: 10 }
    }
