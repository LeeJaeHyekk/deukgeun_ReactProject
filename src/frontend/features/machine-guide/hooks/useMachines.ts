...
import type { MachineDTO } from "../../../types/dto/index"
...
export const useMachines = () => {
  ...
  const fetchMachines = useCallback(async () => {
    ...
    await withLoading(async () => {
      ...
      setMachines(result.machines as unknown as MachineDTO[])
      setCachedData(cacheKey, result.machines as unknown as MachineDTO[])
      setCurrentFilter("")
  ...
  const createMachine = useCallback(
    async (machineData: any) => {
      await withLoading(async () => {
        ...
        // 새로 생성된 머신을 목록에 추가
        setMachines(prev => [...prev, result.machine as unknown as MachineDTO])
        setLastFetchTime(Date.now())
        return result
      })
    },
  ...
  const updateMachine = useCallback(
    async (id: number, machineData: any) => {
      await withLoading(async () => {
        ...
        // 수정된 머신을 목록에서 업데이트
        setMachines(prev =>
          prev.map(machine => (machine.id === id ? result.machine as unknown as MachineDTO : machine))
        )
        setLastFetchTime(Date.now())
        return result
      })
